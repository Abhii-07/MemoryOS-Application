...
## Next Actions
1. If deploying: hosted providers need cloud keys in `server/.env` (Ollama local only) — DB on Neon? server on Render/Railway? static site on Vercel? User decision pending; nothing pushed without user approval.
